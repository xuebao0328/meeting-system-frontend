<template>
  <div class="enhanced-preview" :class="{'fullscreen-mode': isFullscreen}">
...
export default {
  ...
  data() {
    return {
      ...
      deviceType: this.$route.query.device || 'desktop',
      pages: [],
      previewUrl: '',
      isFullscreen: false,
      hideControls: false,
      ...
      controlsTimer: null,
      orientation: this.$route.query.orientation || 'portrait',
      urlParams: {}
    }
  },
  ...
  methods: {
    fetchPages() {
      // 模拟获取页面列表，实际项目中应该通过API获取
      this.$http.get(`/api/meetings/${this.meetingId}/pages`).then(response => {
    ...
    updatePreview() {
      ...
      // 更新路由参数，不刷新页面
      this.$router.replace({
        path: `/preview/${this.meetingId}/${this.pageId}`,
        query: {
          device: this.deviceType,
          orientation: this.orientation,
          fullscreen: this.isFullscreen ? 'true' : undefined
        }
      });
    },
    ...
    updatePreviewUrl() {
      ...
      this.previewUrl = `/api/meetings/${this.meetingId}/website/preview?${queryString}`;
    },
    ...
    copyPreviewLink() {
      ...
      const previewLink = `${baseUrl}/preview/${this.meetingId}/${this.pageId}?device=${this.deviceType}&orientation=${this.orientation}`;
...
.fullscreen-mode {
  ...
  .device-simulator {
    ...
    &.device-iphone14-pro, &.device-iphone13, &.device-iphoneSE, &.device-android {
      transform: scale(0.9);
    }
